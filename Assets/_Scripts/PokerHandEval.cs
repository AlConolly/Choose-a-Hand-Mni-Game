...
static class PokerHandEval
    {
    public static string[] suits = Card.suits;
    ...
    public void TestEval()
    {
        ...
        print(GetPlayerHandRank(list));
        
    }*/
        public static double EvaluateRankByHighestCards(List<string[]> cards, int excludeCardValue = -1, int excludeCardValue2 = -1, int limitCheck = 5, double normalize = 433175)
        {
            int i = 0;
            double sum = 0;
        ...
        public static HandRank GetPlayerHandRank(List<string[]> FiveCards)
        {
        ...
        FiveCards.Sort((x, y) =>
            ...
            maxCardValue = Int32.Parse(FiveCards[4][0]);
            ...
            for (int i = 0; i < 4; i++)
            {
                currCardValue = Int32.Parse(FiveCards[i][0]);
                currCardSuit = FiveCards[i][1];

                nextCardValue = Int32.Parse(FiveCards[i + 1][0]);
                nextCardSuit = FiveCards[i + 1][1];
            ...
            if (FiveCards[4][0] == "14" && FiveCards[3][0] == "13" && FiveCards[2][0] == "12" && FiveCards[1][0] == "11" && FiveCards[0][0] == "10"
                && FiveCards[4][1] == FiveCards[3][1] && FiveCards[4][1] == FiveCards[2][1] && FiveCards[4][1] == FiveCards[1][1] && FiveCards[4][1] == FiveCards[0][1])
            {
                rankName = "Royal King";
                rank = 2000;

                rankCards.AddRange(FiveCards);
            }
            else
            {
                ...
                {
                    var suitCards = FiveCards.Where(x => x[1].Equals(suit)).ToList();
                    if (suitCards.Count() >= 5)
                    {
                        ...
                        if (counter >= 5)
                        {
                            ...
                            rank = 1000;// + (double)lastValue / 14 * 99;
                        }
                        ...
                        {
                            rankName = "Straight Flush";
                            rank = 1000;//835.3571; // The result of: 800 + 5 / 14 * 99
                        }
                    }
                }
                ...
                if (rankName == null)
                {
                    ...
                    if (duplicates.Count > 0 && duplicates[0][0] == 4)
                    {
                        ...
                        rank = 700; //+ duplicates[0][1] / 14 * 50 + EvaluateRankByHighestCards(FiveCards, (int)duplicates[0][1], -1, 1);
                    ...
                    else
                    {
                        ...
                        foreach (string suit in suits)
                        {
                            var suitCards = FiveCards.Where(x => x[1].Equals(suit));
                        ...
                        if (rankName == null)
                        {
                            ...
                            rank = 400;//435.3571; // The result of 400 + 5/14 * 99
                            ...
                            rank = 300;// + duplicates[0][1] / 14 * 50 + EvaluateRankByHighestCards(FiveCards, (int)duplicates[0][1]);
                            ...
                            else if (duplicates.Count > 1 && duplicates[0][0] == 2 && duplicates[1][0] == 2)
                            {
                                ...
                                rank = 200;// + (Math.Pow(threePairsValues[0], 2) / 392 + Math.Pow(threePairsValues[1], 2) / 392) * 50 + EvaluateRankByHighestCards(FiveCards, (int)threePairsValues[0], (int)threePairsValues[1]);
                            ...
                            {
                                rankName = "Pair";
                            rank = 100;// + duplicates[0][1] / 14 * 50 + EvaluateRankByHighestCards(FiveCards, (int)duplicates[0][1], -1, 3);
                            ...
                            {
                                rankName = "High Card";
                            rank = 0;// EvaluateRankByHighestCards(FiveCards, -1, -1, 5);
        ...
        Debug.Log(rankName +"" +rank +""+rankCards);
            return new HandRank
            {
                RankName = rankName,
                Rank = rank,
